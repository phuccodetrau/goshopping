...
function isTokenAlive(token) {
    try {
        ...
        return decoded.exp > currentTime; // true nếu token còn hạn
    } catch (error) {
...
const updatePassword =async(req,res,next)=>{
    try{
        ...
        const user = await AuthService.updatePassword(email,oldPassword,newPassword);
        if(!user.data){
