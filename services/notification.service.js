    ...
    static async createTaskNotification(memberEmail, taskName, type = 'task_assigned', taskDetails = {}) {
        try {
            ...
            if (type === 'task_assigned') {
                title = 'Nhiệm vụ mới';
                content = `[${groupName}] Bạn được giao nhiệm vụ mua ${foodDetails}${dateRange ? ` (${dateRange})` : ''}${taskDetails.note ? `\nGhi chú: ${taskDetails.note}` : ''}`;
            } else if (type === 'task_updated') {
                title = 'Cập nhật nhiệm vụ';
                content = `[${groupName}] Nhiệm vụ của bạn đã được cập nhật: ${foodDetails}${dateRange ? ` (${dateRange})` : ''}${taskDetails.note ? `\nGhi chú: ${taskDetails.note}` : ''}`;
            }
