...
const checkEmailDomain = async (email) => {
    ...
    try {
      ...
      return true;  // Found MX records
    } catch (error) {
      console.error('Error:', error);
      return false;  // No MX records found or an error occurred
    }
}
  const checkEmailExistence = async (email) => {
    try {
      var apiKey=process.env.CHECKEMAIL_API_KEY
      ...
      if (data.deliverability === 'UNDELIVERABLE') {
        return false; // Nếu email không thể nhận thư, trả về false
      } else {
        return true; // Nếu email hợp lệ và có thể nhận thư, trả về true
      }
    } catch (error) {
...
class AuthService{
   ...
   static async register(email, password,name){
      try {
        ...
        if(!isValidEmail(email)||!await checkEmailDomain(email)||!await checkEmailExistence(email)){
    ...
    static async updatePassword(email,oldPassword,newPassword){
        try {
            ...
            if(!isValidPassword){
                ...
                return {message:"Cập nhật mật khẩu thành công",data:"abc"}
            }
        } catch (error) {
            throw error;
        }
